        ...
        <main className="page">
            ...
            <section className="section-hero">
                <div className="hero-left">
                    <Avatar
                        ...
                        alt={`Avatar de ${profile?.personName}`}
                ...
                <div className="academic-list">
                    {profile?.academic?.map((it, idx) => (
                        <div key={idx} className="academic-item">
                            <H3>{it.title}</H3>
                            <Paragraph color="var(--color-muted)">
                                {it.institution}
                            </Paragraph>
                            <Paragraph color="var(--color-muted)">
                                {it.year}
                    ...
                    {profile?.experience?.map((ex, idx) => (
                        <article key={idx} className="exp-item">
                            <H3 className="exp-heading">
                                {ex.heading}{" "}
                                <span className="exp-years">({ex.years})</span>{" "}
                                :
                            </H3>
                            <Paragraph className="exp-desc">
                                {ex.description}
                            </Paragraph>
                        </article>
                    ))}
            ...
            <section className="section-skills">
                ...
                <div className="skills-actions">
                    ...
                    <LinkButton
                        text={
                            pageStrings?.buttons?.download_cv ?? "Descargar CV"
                        }
                        href={
                            profile?.cv?.downloadUrl ||
                            profile?.cv?.viewUrl ||
